...
enum MeasurementMode {
  CONTINUOUS_READ_VBGY = 0,
  CONTINUOUS_READ_GYOR = 1,
  CONTINUOUS_READ_ALL = 2,
  ON_REQUEST_READ_ALL = 3,
};

class AS726X {
public:
	AS726X();
	bool begin(TwoWire &wirePort = Wire, uint8_t gain = 3, MeasurementMode measurementMode = ON_REQUEST_READ_ALL);
...
private:
	...
	bool virtualReadRegister(uint8_t virtualAddr, uint8_t &read_val);
	void virtualWriteRegister(uint8_t virtualAddr, uint8_t dataToWrite);
	void writeRegister(uint8_t addr, uint8_t val);
	bool readRegister(uint8_t addr, uint8_t& read_val);
