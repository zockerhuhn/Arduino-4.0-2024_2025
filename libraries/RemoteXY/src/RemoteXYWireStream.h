...
class CRemoteXYWireStream : public CRemoteXYWire, public CRemoteXYReadByteListener {
  ...
  void readByte (uint8_t byte) override {
    ...
    if (receiveIndex >= receiveBufferSize) {
      RemoteXYDebugLog.write("receiveBufferSize exceeded");
    ...
    RemoteXYDebugLog.write("receiveBuffer write");
  ...
  void receivePackage () {
    if (receiveModified) {
      ...
      uint16_t packageLength;
      si = 0;
      RemoteXYDebugLog.write("receiveIndex: ");
      RemoteXYDebugLog.writeHex(receiveIndex);
      while (si + REMOTEXY_PACKAGE_MIN_LENGTH <= receiveIndex) {   
        RemoteXYDebugLog.write("received package");
